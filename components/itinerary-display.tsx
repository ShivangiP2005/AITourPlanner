...
export default function ItineraryDisplay({ data, destination }: { data: ItineraryData; destination: string }) {
  ...
  const handleDownload = () => {
    let text = `${data.destination || destination} Itinerary\n${data.summary || ""}\n\n`
    ...
    text += `Total Budget: ${data.totalBudget || "N/A"}\nEmergency: ${data.emergencyContacts || "N/A"}\n`

    const element = document.createElement("a")
    const file = new Blob([text], { type: "text/plain" })
    element.href = URL.createObjectURL(file)
    element.download = `itinerary-${data.destination || destination}.txt`
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div>
          <h2 className="text-2xl font-bold">{data.destination || destination}</h2>
        ...
        <Card key={day.day} className="p-6">
          ...
          {day.restaurant && (
            <div className="flex gap-3 mb-3 items-start">
              ...
              <p className="text-sm">
                <span className="font-medium">{day.restaurant.name}</span>
                {day.restaurant.cuisine && ` — ${day.restaurant.cuisine}`}
                {day.restaurant.estimatedCost && `, ${day.restaurant.estimatedCost}`}
              </p>
            </div>
          )}
      ...
      {(data.totalBudget || data.emergencyContacts || (data.localPhrases && data.localPhrases.length > 0)) && (
        <Card className="p-6 bg-muted/50">
          ...
          {data.localPhrases && data.localPhrases.length > 0 && (
            <div className="mt-4">
